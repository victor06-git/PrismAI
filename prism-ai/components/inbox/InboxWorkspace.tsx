        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          ...
          <main className="relative flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden border-x border-prisma-border bg-prisma-surface">
            ...
            {status === "tasks" ? (
              ...
              visual?.imageUrl ? <div className="flex min-h-0 flex-1 items-center justify-center overflow-auto px-8 py-8 pb-24"><figure className="w-full max-w-2xl"><img src={visual.imageUrl} alt={visual.assetName} className="aspect-video w-full rounded-md border border-prisma-border object-cover" /><figcaption className="mt-3 text-sm text-prisma-muted">{visual.assetName}</figcaption></figure></div> : <EmptyState icon={ImageIcon} title={meeting.error ? "Moodboard generation failed" : meeting.phase === "review" && visual ? "Generating moodboard…" : "No moodboard yet"} body={meeting.error ?? "One optimized Fal image will appear here without blocking the summary."} loading={!meeting.error && meeting.phase === "review" && Boolean(visual)} />
