...
function EmptyState({ icon: Icon, title, body, action, loading = false }: { icon: typeof Mic; title: string; body: string; action?: () => void; loading?: boolean }) {
  return <div className="flex flex-1 flex-col items-center justify-center px-8 pb-24"><div className="mb-5 flex h-14 w-14 items-center justify-center rounded-full border border-prisma-border bg-prisma-canvas">{loading ? <div className="h-6 w-6 animate-spin rounded-full border-2 border-prisma-border border-t-prisma-text" /> : <Icon className="h-6 w-6 text-prisma-muted stroke-[1.5]" />}</div><h2 className="font-serif text-2xl tracking-tight text-prisma-text">{title}</h2><p className="mt-2 max-w-sm text-center text-sm leading-relaxed text-prisma-muted">{body}</p>{action && <button type="button" onClick={action} className="mt-5 rounded-full bg-prisma-text px-4 py-2 text-sm text-white">Start recording</button>}</div>;
}
